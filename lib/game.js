...
const buy = async (m, args, db) => {
	if (args[0] === 'limit') {
		...
		if (db.users[m.sender].uang >= count * 1) {
			...
			db.users[m.sender].uang -= count * 10000
			m.reply(`Berhasil Membeli Limit Sebanyak ${args[1] * 1} dengan harga ${args[1] * 10000}`);
		} else m.reply(`Uang Kamu Tidak Cukup Untuk Membeli limit!\nUangmu Tersisa : ${db.users[m.sender].uang}\nHarga ${args[1]} Limit : ${args[1] * 10000}`);
	} else m.reply(`Harga Limit : Jumlah x 10000\n• 1 limit = 10000\n• 2 limit = 20000\n\nExample : .buy limit 3`);
}
